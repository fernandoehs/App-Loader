...
class MainActivity : AppCompatActivity() {
    ...
    private fun sendNotification(){
        ...
        notificationManager.sendNotification(
            "Download complete", applicationContext, detailPendingIntent)
    }
    ...
    fun radioButtonClick(view: View){
        if (view is RadioButton){
            ...
            when(view.getId()){
                R.id.radio_item_1 ->
                    if(checked){ targetURL = GLIDE_URL}
                R.id.radio_item_2 ->
                    if(checked){ targetURL = UDACITY_URL}
                R.id.radio_item_3 ->
                    if(checked){ targetURL = RETROFIT_URL}
            }
        }
    }
    ...
    private fun createChannel(channelId:String, channelName:String){
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            ...
            notificationChannel.description ="Download Reminder"
